<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('cars', function (Blueprint $table) {
            $table->id();
            $table->foreignId('car_model_id')->constrained('car_models')->restrictOnDelete()
                ->comment('Связь с моделью автомобиля');

            $table->foreignId('driver_id')->constrained('drivers')->restrictOnDelete()
                ->comment('Водитель, закреплённый за автомобилем');

            $table->string('license_plate')
                ->unique()
                ->comment('Государственный регистрационный номер автомобиля');

            $table->unsignedSmallInteger('year')
                ->comment('Год выпуска автомобиля');

            $table->string('color')
                ->comment('Цвет автомобиля');

            $table->enum('status', ['available', 'maintenance'])
                ->default('available')
                ->comment('Статус автомобиля: available — доступен, maintenance — на обслуживании');

            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('cars');
    }
};
